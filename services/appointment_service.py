...
def create_appointment(
    ...
    *,
    ...
    hospital = fetch_one(conn, "SELECT name AS hospital_name, city AS hospital_city FROM hospitals WHERE id=?", (hospital_id,)) or {}
...
def list_appointments(conn: Any, user_id: int) -> list[dict[str, Any]]:
    return fetch_all(
        conn,
        """
        SELECT a.*, h.name AS hospital_name, h.city AS hospital_city, h.specialization
        FROM appointments a
